...
void printAttributes(char name[], struct stat statBuff) {
    ...
    // Evaluate file mode for owner permissions
    ...
    // Evaluate file mode for group permissions
    ...
    // Evaluate file mode for other permissions
    ...
    printf("Date created: %s", ctime(&statBuff.st_ctime));
    printf("Date modified: %s", ctime(&statBuff.st_mtime));
    printf("Time file was last accessed: %s", ctime(&statBuff.st_atime));
    printf("File size: %lld bytes\n", statBuff.st_size);
}

//Threading Stuff

// Function prototype for multiThreads()
void multiThreads();
...
void* threadFunction(void* vargp) {
    ...
    int* myid = (int*) vargp;
    printf("\n------------------------");
    printf("Thread #%d has started.", *myid);
    printf("------------------------\n");
    for (i = 0; i < LOOP; i++) {
        printf("Thread #%d is printing the current loop iteration: %d", *myid, i);
        if (i != LOOP-1){
            printf("\n");
        }
    }
    printf("\n------------------------");
    printf("Thread #%d has finished.", *myid);
    printf("------------------------\n");
    return NULL;
}
